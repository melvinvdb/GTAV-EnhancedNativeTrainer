...
void upgradeVehMaximum(Vehicle veh)
{
	...
	if (VEHICLE::IS_VEHICLE_A_CONVERTIBLE(veh, false)) // -- Roof
	{
		VEHICLE::LOWER_CONVERTIBLE_ROOF(veh, true);
	}

	VEHICLE::SET_VEHICLE_WHEEL_TYPE(veh, WHEEL_TYPE_HIGHEND); // -- Wheel cat/type
	int max = VEHICLE::GET_NUM_VEHICLE_MODS(veh, MOD_FRONTWHEELS);
	VEHICLE::SET_VEHICLE_MOD(veh, MOD_FRONTWHEELS, rand() % max, 0);
	VEHICLE::SET_VEHICLE_MOD(veh, MOD_BACKWHEELS, rand() % max, 0);

	VEHICLE::SET_VEHICLE_WINDOW_TINT(veh, WINDOWTINT_LIGHTSMOKE); // -- Window tint

	char* playerName = PLAYER::GET_PLAYER_NAME(PLAYER::PLAYER_ID());
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT_INDEX(veh, PLATE_YANKTON); // -- NumberPlates
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(veh, playerName);

	addClanLogoToVehicle(veh, PLAYER::PLAYER_PED_ID());
